...
void Simple::defrag() {
  ...
  for (auto it = _alloced.begin(); it != _alloced.end(); ++it) {
    ...
    for (auto p_it = Pointer::_pointers.begin();
         p_it != Pointer::_pointers.end(); ++p_it) {

      char *real_p = static_cast<char *>((*p_it)->_pos);
      if (size_t(real_p) == (size_t(_base) + cur_p_start)) {
        for (size_t cur_pos_used = cur_min; cur_pos_used < cur_min + cur_size;
             ++cur_pos_used) {
